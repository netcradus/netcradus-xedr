...
def upgrade() -> None:
    ...
    if not insp.has_table("vuln_scans"):
        op.create_table(
            "vuln_scans",
        ...
        existing_idx = {i['name'] for i in insp.get_indexes("vuln_scans")}
        if "ix_vuln_scans_agent_id" not in existing_idx:
            op.create_index("ix_vuln_scans_agent_id", "vuln_scans", ["agent_id"])
        if "ix_vuln_scans_tenant_id" not in existing_idx:
            op.create_index("ix_vuln_scans_tenant_id", "vuln_scans", ["tenant_id"])

    if not insp.has_table("vuln_findings"):
        op.create_table(
            "vuln_findings",
            sa.Column("id",                 sa.Integer, primary_key=True),
        ...
        existing_idx = {i['name'] for i in insp.get_indexes("vuln_findings")}
        for idx_name, col in [
            ("ix_vuln_findings_tenant_id",  ["tenant_id"]),
            ("ix_vuln_findings_agent_id",   ["agent_id"]),
            ("ix_vuln_findings_severity",   ["severity"]),
            ("ix_vuln_findings_check_type", ["check_type"]),
            ("ix_vuln_findings_status",     ["status"]),
            ("ix_vuln_findings_cve_id",     ["cve_id"]),
        ]:
            if idx_name not in existing_idx:
                op.create_index(idx_name, "vuln_findings", col)
